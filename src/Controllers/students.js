...
const creditUpdate = async (id, amount, method) => {
  ...
  if (method == "credit") {
    ...
    let student = await Student.findByIdAndUpdate(
      id,
      { $inc: { wallet: amount / 4, credits: decrease } },
      { new: true }
    );
    return student;
  }
...
const apicredits = async (req, res) => {
  let student = await creditUpdate(
    req.user._id,
    req.body.amount,
    req.body.method
  );
...
const apiwallet = async (req, res) => {
  let student = await walletUpdate(
    req.user._id,
    req.body.amount,
    req.body.method
  );
...
const delBookmarks = async (req, res) => {
  ...
  student = await student.save();
  if (beforeLen === student.bookmarks.length)
    throw new Error("There are no bookamrks based on this ID");
...
const del_history = async (req, res) => {
  ...
  student.history = student.history.filter(
    (item) => String(item._id) !== req.params.id
  );
  student = await student.save();
  if (beforeLen === student.history.length)
    throw new Error("There are no history based on this ID");
